...
/* Casts a ray to determine if it hits any objects in the scene. Uses Phong Shading.
*	@r: The ray to cast.
...
*/
vec3 raycastMaterial(const ray& r, const hittable& world, int depth) {
    hit_record rec;
    ...
    if (world.hit(r, 0.001, infinity, rec)) {
        ...
        if (rec.mat_ptr->scatter(r, rec, attenuation, scattered))
            return attenuation * raycastMaterial(scattered, world, depth-1);
        return vec3(0,0,0);
    }
...
int main(int argc, char** args) {
	...
	// perspective
	//world.add(make_shared<triangle>(vec3(50-100,50-100,100), vec3(0-100,-50-100,100), vec3(100-100,-50-100,100), vec3(0.5,0.4,0.8), vec3(1,1,1)));
	...
	auto material_glass	 = make_shared<dielectric>(1.23);

	//world.add(make_shared<triangle>(vec3(50-100,50-100,-10), vec3(0-100,-50-100,-10), vec3(100-100,-50-100,-10), vec3(0.5,0.4,0.8), vec3(1,1,1), material_center));

    /*world.add(make_shared<sphere>(vec3( 0.0, -100.5, -1.0), 100.0, vec3(0,0,0), material_ground));
    world.add(make_shared<sphere>(vec3( 0.0,    0.0, -1.0),   0.5, vec3(0,0,0), material_center));
    world.add(make_shared<sphere>(vec3(-1.0,    0.0, -1.0),   0.5, vec3(0,0,0), material_left));
    world.add(make_shared<sphere>(vec3( 1.0,    0.0, -1.0),   0.5, vec3(0,0,0), material_right));*/

	//world.add(make_shared<sphere>(vec3( 0.0, 100, -150), 100.0, vec3(0,0,0), material_left));
	world.add(make_shared<sphere>(vec3( 0.0, 0, -100), 80, vec3(0,0,0), material_glass));
	world.add(make_shared<sphere>(vec3( 0.0, 100, -300), 100.0, vec3(0,0,0), material_ground));
	//world.add(make_shared<sphere>(vec3( 0, -300.5, -150), 300.0, vec3(0,0,0), material_ground));

    //world.add(make_shared<sphere>(vec3( 0.0,    0.0, 0),   50, vec3(0,0,0), material_center));
   	//world.add(make_shared<sphere>(vec3(-1.0,    0.0, 0),   50, vec3(0,0,0), material_left));
    //world.add(make_shared<sphere>(vec3( 1.0,    0.0, 0),   50, vec3(0,0,0), material_right));

	//world.add(make_shared<sphere>(vec3(-50,0,-60),49.99,vec3(0,0,1), material_ground));
	//world.add(make_shared<plane>(vec3(0,0,-400), vec3(0,0,1), vec3(0.8,0.5,0.8), material_center));	// plane doesnt have any axis aligned bounding box so dont use it in MP2
	//world.add(make_shared<sphere>(vec3( 0, -300.5, -150), 300.0, vec3(0,0,0), material_ground));
	...
	//world = generateSpheres(100, image_width, image_height, 5); (MP2)
	...
	const int samples_per_pixels = 64;	// 100, must be square of some number
        ...
        for (int i = 0; i < image_width; ++i) {
			...
			for (int k = 0; k < samples_per_pixels; k++) {
				...
				if (!materialShading) {
					...
					color += raycastMaterial(r,world,100);
					//color += raycastMaterial(r,bvh, 50);
				}
			}
